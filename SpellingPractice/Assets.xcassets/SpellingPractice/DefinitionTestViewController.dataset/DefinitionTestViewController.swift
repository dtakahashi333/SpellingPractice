    ...
    @IBAction func confirmButtonPressed(_ sender: Any) {
        ...
        if let yourAnswer = answerChoice1.selected() {
            ...
            for i in 0 ..< checkFields.count {
                if (i == answerIndex) {
                    checkFields[i].text = String.fontAwesomeIcon("check")
                    checkFields[i].textColor = cloverColor
                } else {
                    checkFields[i].text = String.fontAwesomeIcon("times")
                    checkFields[i].textColor = UIColor.red
                }
                checkFields[i].font = UIFont.icon(from: .fontAwesome, ofSize: 17.0)
            }
        }
